...
const recommendationPrompt = ({ mode, snapshot }) => {
  ...
  return [
    modeInstruction,
    'Use only the provided trip snapshot. Do not claim you checked live availability, opening hours, prices, maps, or external place data.',
...
const normalizeRecommendation = (recommendation, index, context = {}) => {
  ...
  const title = cleanText(source.title || placeDraft.name || eventDraft.title, 120);
  const locationText = cleanText(source.locationText || placeDraft.address || eventDraft.location || title, 180);
  ...
  return {
    ...
    placeDraft: {
      name: cleanText(placeDraft.name || title || locationText, 120),
      address: cleanText(placeDraft.address || locationText || title, 180),
      note: cleanText(placeDraft.note || source.reason, 280)
    },
    eventDraft: {
      title: cleanText(eventDraft.title || title || locationText, 120),
      location: cleanText(eventDraft.location || locationText || title, 180),
