...
const EventCard = ({ event, prevLocation, onEdit, onDelete, onUpdateMemos, onOpenGoogleMaps, onViewDetails, onAddExpense }) => {
  ...
  return (
    <div className="relative pl-6 pb-8 last:pb-0 border-l-2 border-gray-200 dark:border-slate-800 ml-3 group">
      ...
      <article className="bg-white dark:bg-slate-900 rounded-xl p-5 shadow-sm border border-transparent hover:shadow-md transition-all relative group">
        <header className="flex justify-between items-center">
          ...
          <button
            ...
            className="p-1.5 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-full text-gray-400 transition-colors"
            aria-label="更多操作"
          >
        ...
        <div className="mt-3 space-y-2">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white leading-tight break-words">{event.title}</h3>
          {summaryParts.length > 0 && (
            <p className="text-sm text-gray-600 dark:text-slate-300">{summaryParts.join(' · ')}</p>
          )}
        </div>
        ...
        <div className="mt-4 flex items-center justify-between gap-4">
          <button
            ...
            className="inline-flex items-center text-sm font-semibold text-brand-600 dark:text-brand-400 hover:text-brand-700"
          >
            查看詳情
          ...
          <button
            ...
            className="inline-flex items-center text-xs text-gray-500 hover:text-gray-700 dark:text-slate-400"
          >
            {isExpanded ? '收合次要資訊' : '展開次要資訊'}
