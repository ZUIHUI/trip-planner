...
const defaultHelperText = '搜尋或手動輸入地點。';
const defaultEmptyMessage = '找不到地點，可手動輸入。';
const defaultApiUnavailableMessage = '地點搜尋暫時不可用。';

const STATUS_MESSAGES = {
  [GOOGLE_PLACE_PREDICTION_STATUS.loadingFailed]: '地點搜尋暫時失敗。',
  [GOOGLE_PLACE_PREDICTION_STATUS.apiNotActivated]: '地點搜尋暫時不可用。',
  [GOOGLE_PLACE_PREDICTION_STATUS.apiTargetBlocked]: '地點搜尋暫時不可用。',
  [GOOGLE_PLACE_PREDICTION_STATUS.billingOrKeyError]: '地點搜尋暫時不可用。',
  [GOOGLE_PLACE_PREDICTION_STATUS.requestFailed]: '地點搜尋暫時失敗。',
  [GOOGLE_PLACE_PREDICTION_STATUS.legacyFallbackUsed]: '已取得地點。'
};
...
const GooglePlaceInput = ({
  ...
  const statusMessage = useMemo(() => {
    ...
    if (query.length < MIN_QUERY_LENGTH) return '再輸入 1 個字。';
    ...
    if (suggestions.length > 0) return '選擇地點後按 Enter。';
    return helperText;
  }, [
